import type { Metadata } from "next";
import { notFound } from "next/navigation";
import Link from "next/link";
import { getNationalRanking, type RankingSort } from "@/lib/cities";
import { STATES } from "@/lib/states";
import RankedTable from "@/components/ranking/RankedTable";

export const revalidate = 86400;

type VariantSlug =
  | "cheapest-cities"
  | "most-expensive-cities"
  | "highest-income-cities"
  | "cheapest-rent-cities";

type VariantConfig = {
  slug: VariantSlug;
  sort: RankingSort;
  /** Title-cased label used in the tab and for cross-linking. */
  label: string;
  /** Short plain-English descriptor used in the hero sub-headline. */
  hint: string;
  /** <h1> copy — keyword-rich. */
  h1: string;
  /** <title> copy — keyword-rich, suffixed with "| UrbRank". */
  title: string;
};

const VARIANTS: VariantConfig[] = [
  {
    slug: "cheapest-cities",
    sort: "cheapest",
    label: "Cheapest",
    hint: "Lowest cost-of-living index",
    h1: "Cheapest cities to live in the US",
    title:
      "Cheapest Cities to Live in the US (2026) — Ranked by Cost of Living | UrbRank",
  },
  {
    slug: "most-expensive-cities",
    sort: "expensive",
    label: "Most expensive",
    hint: "Highest cost-of-living index",
    h1: "Most expensive cities in the US",
    title:
      "Most Expensive US Cities (2026) — Ranked by Cost of Living | UrbRank",
  },
  {
    slug: "highest-income-cities",
    sort: "income-high",
    label: "Highest incomes",
    hint: "Highest median household income",
    h1: "Highest-income cities in the US",
    title:
      "Highest-Income US Cities (2026) — Ranked by Median Household Income | UrbRank",
  },
  {
    slug: "cheapest-rent-cities",
    sort: "rent-low",
    label: "Cheapest rent",
    hint: "Lowest median rent",
    h1: "Cities with the cheapest rent in the US",
    title:
      "Cheapest Rent in the US (2026) — Cities Ranked by Median Rent | UrbRank",
  },
];

type Params = { variant: string };

export async function generateStaticParams(): Promise<Params[]> {
  return VARIANTS.map((v) => ({ variant: v.slug }));
}

function resolve(variant: string): VariantConfig | null {
  return VARIANTS.find((v) => v.slug === variant) ?? null;
}

export async function generateMetadata({
  params,
}: {
  params: Promise<Params>;
}): Promise<Metadata> {
  const { variant } = await params;
  const cfg = resolve(variant);
  if (!cfg) {
    return {
      title: "Ranking not found — UrbRank",
      robots: { index: false, follow: false },
    };
  }

  const cities = await getNationalRanking(cfg.sort, 100);
  const top = cities[0];
  const description = top
    ? `${top.name}, ${top.state_code} tops the ranking. See the top 100 US cities ranked by ${cfg.hint.toLowerCase()}, with housing, salary, and population data for each.`
    : `Top 100 US cities ranked by ${cfg.hint.toLowerCase()}.`;

  return {
    title: cfg.title,
    description,
    alternates: { canonical: `/rankings/${cfg.slug}` },
    openGraph: { title: cfg.title, description, type: "article" },
  };
}

export default async function VariantRankingPage({
  params,
}: {
  params: Promise<Params>;
}) {
  const { variant } = await params;
  const cfg = resolve(variant);
  if (!cfg) notFound();

  const cities = await getNationalRanking(cfg.sort, 100);

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: cfg.h1,
    numberOfItems: cities.length,
    itemListElement: cities.slice(0, 20).map((c, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: `${c.name}, ${c.state_code}`,
      url: `/cost-of-living/${c.slug}`,
    })),
  };

  const breadcrumbJsonLd = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: "/" },
      { "@type": "ListItem", position: 2, name: "Rankings", item: "/rankings" },
      { "@type": "ListItem", position: 3, name: cfg.label },
    ],
  };

  return (
    <div className="mx-auto max-w-6xl px-6 py-8 md:py-12">
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
      />
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbJsonLd) }}
      />

      <nav aria-label="Breadcrumb" className="text-sm text-[var(--muted)]">
        <ol className="flex flex-wrap items-center gap-2">
          <li>
            <Link href="/" className="hover:text-[var(--foreground)]">
              Home
            </Link>
          </li>
          <li aria-hidden>/</li>
          <li>
            <Link href="/rankings" className="hover:text-[var(--foreground)]">
              Rankings
            </Link>
          </li>
          <li aria-hidden>/</li>
          <li className="text-[var(--foreground)] font-medium">{cfg.label}</li>
        </ol>
      </nav>

      <section className="mt-8 md:mt-12">
        <p className="text-sm font-medium uppercase tracking-widest text-[var(--accent)]">
          National ranking
        </p>
        <h1 className="mt-3 text-4xl md:text-6xl font-semibold tracking-tight leading-[1.05]">
          {cfg.h1}
        </h1>
        <p className="mt-4 max-w-2xl text-lg text-[var(--muted)]">
          {cities.length > 0
            ? `Top ${cities.length} US cities ranked by ${cfg.hint.toLowerCase()}, based on the latest Census ACS and BLS data.`
            : `Rankings will appear here once the ingest scripts run.`}
        </p>
      </section>

      <nav
        aria-label="Other rankings"
        className="mt-8 flex flex-wrap gap-2 border-b border-[var(--border)] pb-3"
      >
        {VARIANTS.map((v) => {
          const active = v.slug === cfg.slug;
          return (
            <Link
              key={v.slug}
              href={`/rankings/${v.slug}`}
              aria-current={active ? "page" : undefined}
              className={`inline-flex flex-col rounded-lg px-4 py-2 text-sm transition-colors ${
                active
                  ? "bg-[var(--accent)] text-white"
                  : "border border-[var(--border)] hover:border-[var(--accent)] hover:text-[var(--accent)]"
              }`}
            >
              <span className="font-medium">{v.label}</span>
              <span
                className={`text-xs ${active ? "text-white/80" : "text-[var(--muted)]"}`}
              >
                {v.hint}
              </span>
            </Link>
          );
        })}
      </nav>

      <section className="mt-6">
        <RankedTable
          cities={cities}
          anchorSlug={cities[0]?.slug}
          showState
        />
      </section>

      <section className="mt-16">
        <h2 className="text-2xl md:text-3xl font-semibold tracking-tight">
          By state
        </h2>
        <p className="mt-2 text-[var(--muted)]">
          See how cities stack up inside each state.
        </p>
        <ul className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
          {STATES.map((state) => (
            <li key={state.code}>
              <Link
                href={`/cheapest-cities/${state.slug}`}
                className="block rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm hover:border-[var(--accent)] hover:text-[var(--accent)] transition-colors"
              >
                {state.name}
              </Link>
            </li>
          ))}
        </ul>
      </section>
    </div>
  );
}
