...
const RAW: Array<Omit<StateInfo, "slug">> = [
...
export const STATES: StateInfo[] = RAW.map((s) => ({
  ...s,
  slug: slugify(s.name),
}));
...
/** Other states in the same Census region, excluding `excludeCode`. */
export function regionPeers(code: string, limit = 5): StateInfo[] {
  const self = getStateByCode(code);
  if (!self) return [];
  return STATES.filter((s) => s.region === self.region && s.code !== self.code).slice(
    0,
    limit,
  );
}
